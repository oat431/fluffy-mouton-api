...
> **References:** [Flowero Guard](../../orlita_md/Personal%20Project/Flowero%20Guard.md) · [Flowero Gate](../../orlita_md/Personal%20Project/Flowero%20Gate.md)  
...
| Decision | Rationale |
|----------|-----------|
| **Option B: Validate JWT independently** | Defense-in-depth. Service has its own JWT validation via Keycloak JWKS. Works even if accidentally exposed without gateway. |
| **Also read `X-User-Id` header** | Convenience — no need to parse JWT in every controller. Gateway handles claim extraction; service trusts internal-network headers. |
...
| **ADD** | OAuth middleware | `internal/middleware/oauth_middleware.go` (new) |
| **ADD** | Go dependency: `github.com/MicahParks/keyfunc` | `go.mod` |
| **MODIFY** | Middleware wiring in router | `internal/router/main_router.go` |
| **MODIFY** | Short-link router (swap middleware) | `internal/router/short_link_router.go` |
...
| **MODIFY** | Docker Compose networking | `docker-compose.*.yml` |
| **MODIFY** | Environment config | `.env.example`, `.env` |
| **REMOVE** | Entire auth module | `controller/auth_controller.go`, `service/auth_service.go`, `repository/auth_repository.go`, `model/auth.go`, `model/refresh_token.go`, `model/verify_token.go`, `repository/refresh_token.go`, `repository/verify_token_repository.go`, `service/email_service.go` |
| **REMOVE** | Old JWT middleware | `internal/middleware/jwt_middleware.go` |
| **REMOVE** | Old JWT utils | `pkg/utils/jwt_util.go` |
| **REMOVE** | Password utils | `pkg/utils/password_util.go` |
| **REMOVE** | Auth router | `internal/router/auth_router.go` |
| **REMOVE** | Auth request/response payloads | `payload/request/login_request.go`, `payload/request/register_request.go`, `payload/response/auth_response.go`, `payload/response/jwt_response.go` |
...
This middleware supports **two auth modes**:
- **Behind Gateway:** Trust `X-User-Id` header (gateway already validated JWT)
- **Direct access:** Validate Keycloak JWT from `Authorization` header
...
//
// Sets c.Locals("user_id") (uuid.UUID) and c.Locals("user_roles") ([]string).
func OAuthMiddleware(c fiber.Ctx) error {
	...
	// ---- Path 2: Direct JWT validation (for service-to-service or direct access) ----
...
// OptionalAuth is a middleware that attempts to extract user info but
// does NOT reject unauthenticated requests. Useful for public endpoints.
...
internal/service/email_service.go       (Keycloak handles email verification)
...
> **⚠️ Important:** The route path changes from `/short-link` to `/short` to align with the gateway path `/api/v1/short/**`. If the old frontend uses `/short-link`, update it or add an alias.
...
### 8.3 Update `internal/config/env_config.go`

No structural changes needed — it just loads `.env`. The new variables will be picked up automatically.
...
The Dockerfile is fine as-is. The healthcheck uses `wget` to hit `/api/v1/health/check` which still exists.
...
### 9.3 Nginx Labels (for routing via Gateway)

If using Docker Compose directly (without K8s), add Nginx labels to expose the service:

```yaml
    labels:
      - "nginx.enable=true"
      - "nginx.http.routers.shortlink.rule=Host(`gateway.panomete.com`) && PathPrefix(`/api/v1/short`)"
      - "nginx.http.routers.shortlink.entrypoints=websecure"
      - "nginx.http.routers.shortlink.tls=true"
      - "nginx.http.routers.shortlink.tls.certresolver=letsencrypt"
      - "nginx.http.services.shortlink.loadbalancer.server.port=8080"
```

However, the **recommended pattern** is to route through Flowero Gate, not expose the service directly via Nginx. The Nginx labels above are for internal routing only.
...
- id: route-shortlink
  ...
  predicates:
    - Path=/api/v1/short/**
  filters:
    - StripPrefix=1                      # /api/v1/short/... → /v1/short/...
    - name: CircuitBreaker
      args:
...
```bash
# 1. Get a token from Keycloak (via gateway login flow)
#    or use client credentials:
TOKEN=$(curl -s -X POST https://auth.panomete.com/realms/flowerogate/protocol/openid-connect/token \
...
# 2. Test via gateway (should return empty array or list)
...
#### Test Direct Service Access (bypassing gateway)

```bash
# 1. Get token from Keycloak directly
TOKEN=$(...) # same as above

# 2. Hit the service directly
...
```bash
curl -s http://localhost:8080/api/v1/health/check
# Expected: 200 OK
```
...
```bash
# Create a short link first, then:
curl -v http://localhost:8080/api/v1/r/<short-code>
# Expected: 301 redirect to target URL
```
...
```bash
# Send 201 rapid requests
for i in $(seq 1 201); do
  curl -s -o /dev/null -w "%{http_code}\n" \
    -H "Authorization: Bearer $TOKEN" \
    https://gateway.panomete.com/api/v1/short/
done
# Expected: First 200 return 200, 201st returns 429
```

#### Test Gateway Circuit Breaker

```bash
# Stop the shortlink-service container
docker stop shortlink-service

# Hit the gateway — should get fallback response after threshold
curl -s https://gateway.panomete.com/api/v1/short/ \
  -H "Authorization: Bearer $TOKEN" \
  | jq

# Expected: 503 with "service_unavailable" message
```
...
[ "$STATUS" = "401" ] && echo "PASS" || echo "FAIL ($STATUS)"
...
RESP=$(curl -sf -X POST "$BASE/random" \
  -H "Authorization: Bearer $TOKEN" \
...
# Test 4: Update short link
echo -n "Test 4 - Update short link... "
STATUS=$(curl -s -o /dev/null -w "%{http_code}" -X PUT "$BASE/$LINK_ID" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
...
STATUS=$(curl -s -o /dev/null -w "%{http_code}" "$BASE/../r/$SHORT")
[ "$STATUS" = "301" ] && echo "PASS" || echo "FAIL ($STATUS)"

# Test 6: Delete short link
echo -n "Test 6 - Delete short link... "
STATUS=$(curl -s -o /dev/null -w "%{http_code}" -X DELETE "$BASE/$LINK_ID" \
  -H "Authorization: Bearer $TOKEN")
[ "$STATUS" = "200" ] && echo "PASS" || echo "FAIL ($STATUS)"

# Test 7: Health check
echo -n "Test 7 - Health check... "
STATUS=$(curl -s -o /dev/null -w "%{http_code}" "https://gateway.panomete.com/api/v1/../health/check")
[ "$STATUS" = "200" ] && echo "PASS" || echo "FAIL ($STATUS)"
...
**Keep `tb_short_link`** — it contains user data that must be retained.
...
This will remove unused dependencies (`golang.org/x/crypto` bcrypt usage, etc.)

### 12.3 Environment Variable Cleanup

Remove from `.env` (if still present):
- `SECRET`
- `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASS`

### 12.4 Update `API_DOCUMENTATION.md`

Remove all auth endpoint documentation sections. Update:
- Authentication section: describe OAuth2 via Keycloak instead of local login
- CORS section: note that CORS is now handled by Flowero Gate
- Code examples: update to use Keycloak token acquisition
- Base URL: change to `https://gateway.panomete.com`

### 12.5 Update `data/db_fluffy_mouton.sql`

Remove `tb_auth`, `tb_refresh_token`, `tb_verify_token` table creation statements. Keep `tb_short_link`.
...
| Email verification gap | Low | Keycloak handles email verification. If email verification was critical, verify it's enabled in Keycloak. |
| Loss of user data | Medium | Old auth table dropped. Users must re-register in Keycloak. |
...
-- 2. Insert mappings (you'll need to match users somehow — email, username, or manual)
--    Example if you can match by username:
...
Phase 8: Update .env                          │  2 min  │
Phase 5: Create keycloak_jwt.go               │ 5 min  │
Phase 5: Create oauth_middleware.go            │ 5 min  │
Phase 5: Update main.go (InitJWKS)            │  2 min  │
Phase 7: Update short_link_controller.go      │  5 min  │ (auth_id → user_id)
Phase 6: Update main_router.go                │  3 min  │
Phase 6: Update short_link_router.go          │  3 min  │
Phase 6: Update api_container.go              │  5 min  │
Phase 6: Remove old auth files                │  2 min  │
Phase 5: go mod tidy                          │  1 min  │
Phase 9: Update docker-compose                │  5 min  │
Phase 10: Add gateway route                   │ 10 min  │
Phase 13: Data migration decision             │ 5 min  │ (truncate or map)
─────────────────────────────────────────────────────────
Phase 11: Deploy & test                       │ 20 min  │
Phase 12: Cleanup                             │  5 min  │
Phase 1: Update API docs                      │ 5 min  │
─────────────────────────────────────────────────────────
                                    Total:   ~2 hours   │
```
...
- [Flowero Guard — Section 8: Downstream Service Integration](../../orlita_md/Personal%20Project/Flowero%20Guard.md#8-downstream-service-integration)
- [Flowero Guard — Section 10: Integration Checklist](../../orlita_md/Personal%20Project/Flowero%20Guard.md#10-integration-checklist-for-new-services)
- [Flowero Gate — Section 13: Integration Checklist](../../orlita_md/Personal%20Project/Flowero%20Gate.md#13-integration-checklist-for-new-services)
